...
class RecipeRepoDB implements IRepo {
  ...
  static const String dbName = "recipes22.db";
  ...
  _onCreate(Database db, int version) async {
    await db.execute("CREATE TABLE $table ($iD INTEGER PRIMARY KEY, $recipeName TEXT, $ingredients TEXT,$cookingSpec TEXT,$time TEXT,$difficulty TEXT, $calories REAL)");
  }
  ...
  Future<Recipe> add(Recipe recipe) async {
    ...
    if (recipe.id == -1) {
      String query = "SELECT MAX(iD) AS r_id FROM $table";
      var a = await dbClient!.rawQuery(query, null);
      // ignore: unnecessary_cast
      var b = a.first as Map<String, dynamic>;
      if (b['r_id'] == null) {
        recipe.id = 0;
      } else {
        recipe.id = b['r_id'] + 1;
